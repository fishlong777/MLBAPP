    ...
    try:
        ...
        if len(names) >= 1:
            with st.spinner('正在分析全球員數據庫...'):
                ...
                # 同時發動多路搜尋
                results = []
                results.append(playerid_lookup(last_raw, first_raw)) # 原始搜
                results.append(playerid_lookup(last_raw))          # 只搜姓

                # 暴力處理所有可能含有重音的情況
                clean_last = normalize_text(last_raw)
                if clean_last != last_raw.lower():
                    results.append(playerid_lookup(clean_last))

                # 合併所有候選清單
                all_candidates = pd.concat(results).drop_duplicates(subset=['key_mlbam']) if results else pd.DataFrame()

                if not all_candidates.empty:
                    # 在本地端進行標準化比對
                    all_candidates['f_norm'] = all_candidates['name_first'].apply(normalize_text)
                    all_candidates['l_norm'] = all_candidates['name_last'].apply(normalize_text)
                    
                    target_f = normalize_text(first_raw)
                    target_l = normalize_text(last_raw)
                    
                    # 篩選最符合的人
                    final = all_candidates[
                        (all_candidates['f_norm'] == target_f) & 
                        (all_candidates['l_norm'] == target_l)
