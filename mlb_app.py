    ...
    try:
        ...
        if len(names) >= 1:
            with st.spinner('連線伺服器中...'):
                # 第一步：只用姓氏搜尋，這是最穩定的 API 調用方式
                l = playerid_lookup(names[-1])
                
                if not l.empty:
                    # 第二步：在本地進行標準化比對
                    l['f_norm'] = l['name_first'].apply(normalize_text)
                    l['l_norm'] = l['name_last'].apply(normalize_text)
                    
                    target_f = normalize_text(names[0])
                    target_l = normalize_text(names[-1])
                    
                    # 篩選：同時匹配姓與名，並取最新的在役球員
                    match = l[(l['f_norm'] == target_f) & (l['l_norm'] == target_l)].sort_values('mlb_played_last', ascending=False).head(1)
