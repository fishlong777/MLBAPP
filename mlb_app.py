...
# --- 1. 核心函數：處理特殊字元 (工業級標準，解決所有重音問題) ---
def normalize_text(text):
    if not isinstance(text, str): return ""
    # NFD 將重音符號拆解，Mn 類別則將其濾除 (例如 Báez -> baez)
...
if st.sidebar.button("查詢資料"):
    try:
        ...
        if len(names) >= 2:
            with st.spinner('連線 MLB 伺服器並進行智慧比對...'):
                last_in, first_in = names[-1], names[0]
                
                # 同時嘗試「原始姓氏」與「強制補正姓氏」來敲開 API 的門
                search_seeds = [last_in]
                manual_corrections = {"baez": "Báez", "alvarez": "Álvarez", "acuna": "Acuña"}
                if last_in in manual_corrections:
                    search_seeds.append(manual_corrections[last_in])
                
                all_results = []
                for seed in list(set(search_seeds)):
                    all_results.append(playerid_lookup(seed))
                
                candidates = pd.concat(all_results).drop_duplicates(subset=['key_mlbam']) if all_results else pd.DataFrame()

                if not candidates.empty:
                    # 本地端脫殼比對：這是解決所有人的終極關鍵
...
if st.session_state.data_cache is not None and st.session_state.p_name != "":
    ...
    st.markdown(f"<h1>{st.session_state.p_name} - {sel_date}</h1><hr>", unsafe_allow_html=True)
